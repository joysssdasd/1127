...
export class SupabaseLedgerRepository implements LedgerRepository {
  ...
  async addTransaction(entry: Omit<PointTransaction, 'id' | 'createdAt'>): Promise<PointTransaction> {
    ...
    const balanceAfter = user.points + entry.amount;
    if (balanceAfter < 0) {
      throw new Error('insufficient balance');
    }
    ...
    const { data: inserted, error: insertError } = await this.client
      .from<PointTransactionRow>('point_transactions')
      .insert({
    ...
    return {
      ...
      changeType: inserted.change_type as PointTransaction['changeType'],
      amount: inserted.amount,
      balanceAfter: inserted.balance_after,
      description: inserted.description ?? '',
      referenceId: inserted.reference_id ?? undefined,
      createdAt: new Date(inserted.created_at),
    };
  }

  async getBalance(userId: string): Promise<number> {
    const { data, error } = await this.client.from<{ points: number }>('users').select('points').eq('id', userId).single();
    if (error || !data) {
      throw error ?? new Error('user not found');
    }
    return data.points;
...
export class InMemoryRechargeRepository implements RechargeRepository {
  private readonly tasks: Map<string, RechargeTask> = new Map();
...
export class SupabaseRechargeRepository implements RechargeRepository {
  ...
  async create(task: Omit<RechargeTask, 'id' | 'status' | 'remindCount' | 'createdAt' | 'updatedAt'>): Promise<RechargeTask> {
    const { data, error } = await this.client
      .from<RechargeRow>('recharge_tasks')
