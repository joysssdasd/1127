...
export class SupabaseContactViewRepository implements ContactViewRepository {
  ...
  async create(record: Omit<ContactViewRecord, 'id' | 'createdAt'>): Promise<ContactViewRecord> {
    ...
    return mapContactRow(data as ContactViewRow);
  }
  ...
  async findById(id: string): Promise<ContactViewRecord | undefined> {
    ...
    return data ? mapContactRow(data as ContactViewRow) : undefined;
  }
...
export class SupabaseDealStatRepository implements DealStatRepository {
  ...
  async increment(postId: string, sellerId: string): Promise<DealStatRecord> {
    const { data } = await this.client
      ...
      .eq('post_id', postId)
      .maybeSingle();
    const existing = data as DealStatRow | null;
    if (!existing) {
      ...
      const row = inserted as DealStatRow;
      return { postId, sellerId, totalDeals: row.total_deals, updatedAt: new Date(row.updated_at) };
    }

    const { data: updated, error: updateError } = await this.client
      .from('deal_stats')
      .update({ total_deals: existing.total_deals + 1, updated_at: new Date().toISOString() })
      .eq('post_id', postId)
      .select('*')
      .single();
    ...
    const row = updated as DealStatRow;
    return { postId, sellerId: row.seller_id, totalDeals: row.total_deals, updatedAt: new Date(row.updated_at) };
  }
}
